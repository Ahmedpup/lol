...
var filter = m => m.author.id === message.author.id;
  if(message.content.startsWith(prefix + "gstart")) { 
    ...
    message.channel.send(`:eight_pointed_black_star:| **اكتب اسم روم القيف اواي**`).then(msg => {
      message.channel.awaitMessages(filter, {
        ...
        msg.edit(':eight_pointed_black_star:| **اكتب بعد كم القيف اواي**').then(msg => {
          message.channel.awaitMessages(filter, {
            max: 1,
            time: 20000,
            errors: ['time']
          }).then(collected => {
            if(!collected.first().content.match(/[1-60][s,m,h,d,w]/g)) return message.channel.send('**!البوت لا يعرف هذا الوقت**');
            duration = collected.first().content
            collected.first().delete();
            msg.edit(':eight_pointed_black_star:| **اكتب ماهي الجائزة**').then(msg => {
              message.channel.awaitMessages(filter, {
                max: 1,
                time: 20000,
                errors: ['time']
                ...
                try {
                  ...
                  message.guild.channels.find("name" , room).send(' :heavy_check_mark: **تم صنع القيف اواي** :heavy_check_mark:' , {embed: giveEmbed}).then(m => {
                     let re = m.react('🎉');
                     setTimeout(() => {
